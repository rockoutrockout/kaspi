...
function TabIcon({ type, className = 'h-[21px] w-[21px]' }) {
...
function DocumentIllustration({ tone = 'green', type = 'id' }) {
  ...
  return (
    <svg className="h-[34px] w-[48px]" viewBox="0 0 54 38" fill="none" aria-hidden="true">
...
function RedServiceIcon({ type }) {
  ...
  return (
    <span className="flex h-10 w-10 shrink-0 items-center justify-center text-[#F14635]">
...
function DocumentCard({ title, tone, type, onClick }) {
  ...
  return (
    <Tag
      ...
      onClick={onClick}
      className="flex h-[84px] min-w-[87px] flex-col justify-between rounded-lg bg-[#F4F5F6] p-2 text-left active:bg-zinc-200"
    >
      <DocumentIllustration tone={tone} type={type} />
      <span className="text-[12px] font-medium leading-[14px] text-zinc-900">{title}</span>
    </Tag>
  );
}
  ...
  const content = (
    <>
      ...
      <div className="min-w-0 flex-1 py-[13px]">
        <p className="text-[15px] font-normal leading-[18px] text-zinc-800">{title}</p>
        {subtitle && <p className="mt-0.5 text-[11px] leading-[14px] text-zinc-400">{subtitle}</p>}
      </div>
        ...
        <header className="sticky top-0 z-20 bg-white pt-safe">
          <div className="relative flex h-[45px] items-center justify-center">
            <button
              ...
              className="absolute left-1 top-0 flex h-[45px] w-[45px] items-center justify-center text-zinc-900 active:opacity-60"
              aria-label="Назад"
            >
              <BackIcon />
            </button>
            <h1 className="text-[16px] font-semibold leading-5 text-zinc-900">Госуслуги</h1>
          </div>
        </header>

        <main className="min-h-0 flex-1 overflow-y-auto pb-[72px] no-scrollbar">
          <div className="bg-white px-3 pb-3">
            <div className="flex h-[34px] rounded-xl bg-[#F3F3F3] p-0.5">
              <button type="button" className="flex flex-1 items-center justify-center rounded-[10px] bg-white text-[13px] font-medium text-zinc-800 shadow-sm active:opacity-80">
                Все услуги
              </button>
              <button type="button" className="flex flex-1 items-center justify-center rounded-[10px] text-[13px] font-medium text-zinc-800 active:bg-zinc-200">
                Мои заявки
              </button>
            </div>
            ...
            <button
              type="button"
              className="mt-3 flex h-[44px] w-full items-center gap-2 rounded-xl bg-[#F4F5F6] px-3 text-left active:bg-zinc-200"
            >
              <SearchIcon className="h-5 w-5 text-zinc-300" />
              <span className="text-[14px] text-zinc-400">Поиск по Госуслугам</span>
            </button>
          </div>
          ...
          <section className="mt-3 bg-white px-3 py-3">
            ...
            >
              <span className="text-[14px] font-semibold leading-5 text-[#1687D9]">Все документы</span>
          ...
          <section className="mt-3 bg-white">
            <div className="border-b border-[#EEF0F2] px-4 pb-2 pt-3">
              <p className="text-[13px] font-semibold leading-4 text-zinc-500">{childName}</p>
            </div>
